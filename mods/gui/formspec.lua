...
gui.colors = "listcolors[#00000069;#5A5A5A;#141318;#30434C;#FFF]"
...
function gui.get_hotbar_itemslot_bg(x, y, w, h)
   ...
   for i = 0, w - 1, 1 do
      for j = 0, h - 1, 1 do
	       out = out .."image["..x+i..","..y+j..";1,1;gui_itemslot_dark.png]"
      end
   end
   return out
end
...
gui.register_tab("inventory", {
  ...
  get = function(name)
    return
      ...
      gui.make_inv(0.25, 4.7, 8, 4, "current_player", "main")
  end,
})
...
gui.register_tab("creative", {
  ...
  get = function(name)
    ...
    return
      "label[6.48,6.7;" .. minetest.colorize("#FFFF00", tostring(pagenum)) .. " / " .. tostring(pagemax) .. "]" ..
      gui.make_inv(0.25, 7.7, 8, 1, "current_player", "main") ..
      gui.make_inv(4.25, 6.5, 1, 1, "detached:trash", "main", false) ..
      "listring[]" ..
      "listring[current_player;main]" ..
      "listring[detached:creative_" .. name .. ";main]" ..
      gui.make_inv(0.25, 0.25, 8, 6, "detached:creative_" .. name, "main", false, tostring(start_i)) ..
      "field[0.55,6.89;2.2,1;creative_filter;;" .. minetest.formspec_escape(inv.filter) .. "]" ..
      gui.make_button(5.65, 6.55, 1, 1, "creative_prev", "<") ..
      gui.make_button(7.34, 6.55, 1, 1, "creative_next", ">") ..
      gui.make_button(2.35, 6.55, 1, 1, "creative_search", "?") ..
      gui.make_button(3, 6.55, 1, 1, "creative_clear", "X") ..
      [[
        image[4.33,6.6;0.8,0.8;gui_icon_trash.png]
