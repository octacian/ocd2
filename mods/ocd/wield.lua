...
local function add_wield_entity(player)
	...
	if name and pos then
		pos.y = pos.y + 0.5
		local object = minetest.add_entity(pos, "ocd:wield_entity")
		if object then
			object:set_attach(player, location[1], location[2], location[3])
			object:set_properties({
				textures = {"ocd:wield_hand"},
...
-- [register] Wield hand
minetest.register_item("ocd:wield_hand", {
	type = "none",
	wield_image = "blank.png",
})

-- [register] Wield entity
minetest.register_entity("ocd:wield_entity", {
	physical = false,
